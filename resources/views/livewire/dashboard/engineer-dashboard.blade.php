<div class="space-y-8">
    ...
    <div class="flex items-center justify-between">
        <div>
            <flux:heading size="xl" level="1">{{ __('Engineer Dashboard') }}</flux:heading>
            <flux:subheading>{{ __('Monitor your assigned ticket queue, track progress, and view recently updated issues.') }}</flux:subheading>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5 shadow-xs flex items-center gap-4">
            ...
            <div>
                <span class="text-xs text-zinc-400 dark:text-zinc-500 font-semibold uppercase tracking-wider block">{{ __('Assigned Tickets') }}</span>
        ...
        <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5 shadow-xs flex items-center gap-4">
            ...
            <div>
                <span class="text-xs text-zinc-400 dark:text-zinc-500 font-semibold uppercase tracking-wider block">{{ __('Open') }}</span>
                <span class="text-2xl font-bold text-zinc-900 dark:text-zinc-50">{{ $metrics['status_counts']['open'] ?? 0 }}</span>
            </div>
        </div>

        <!-- Card: In Progress -->
        <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5 shadow-xs flex items-center gap-4">
            <div class="p-3 bg-orange-50 dark:bg-orange-950/30 text-orange-600 dark:text-orange-400 rounded-lg">
                <flux:icon icon="arrow-path" class="size-6" />
            </div>
            <div>
                <span class="text-xs text-zinc-400 dark:text-zinc-500 font-semibold uppercase tracking-wider block">{{ __('In Progress') }}</span>
                <span class="text-2xl font-bold text-zinc-900 dark:text-zinc-50">{{ $metrics['status_counts']['in_progress'] ?? 0 }}</span>
            </div>
        </div>
    ...
    <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 shadow-xs space-y-4">
        <flux:heading size="md" level="2" class="font-semibold text-zinc-900 dark:text-zinc-50">
            {{ __('Your Queue Status Breakdown') }}
        </flux:heading>
        ...
        <div class="h-4 w-full flex rounded-full overflow-hidden bg-zinc-100 dark:bg-zinc-800">
            ...
            @else
                <div class="w-full bg-zinc-200 dark:bg-zinc-700 flex items-center justify-center text-[10px] text-zinc-400">
                    {{ __('No tickets currently assigned to you') }}
                </div>
            @endif
        </div>
    ...
    <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-xs overflow-hidden">
        <div class="p-6 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
            <flux:heading size="md" level="2" class="font-semibold text-zinc-900 dark:text-zinc-50">
                {{ __('Your Recently Updated Tickets') }}
            </flux:heading>
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-zinc-400 dark:text-zinc-500">
                                <div class="flex flex-col items-center justify-center space-y-2">
                                    ...
                                    <span class="text-sm">{{ __('No tickets currently assigned to you.') }}</span>
                                </div>
                            </td>
                        </tr>
                    @endforelse
